...
void PartnerManager::loadFromDb() {
    ...
    try {
        ...
        dbreq1  << " SELECT pid, uname, pass, cname, manager, balance, credit, plimit, postplay, trial, priority FROM partners;";
        ...
        for ( Result::const_iterator dbr = res.begin(); dbr != res.end(); dbr++ ) {
            ...
            pb.get<1>().insert( PartnerInfo( (*dbr)[1].as<string>(), (*dbr)[2].as<string>(), (*dbr)[0].as<string>(), (*dbr)[3].as<string>(), (*dbr)[4].as<string>(), (*dbr)[5].as<double>(), (*dbr)[6].as<double>(), (*dbr)[7].as<int>(), (*dbr)[8].as<bool>(), (*dbr)[9].as<bool>(), (*dbr)[10].as<int>() ) );
        }
